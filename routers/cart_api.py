...
@route.get("/cart/")
def get_all_item_in_cart(user=Depends(verify_token)):
    """
    ...
    try:
        user_id = user
        wish_list = retrieve_cart_item(user_id[0])
...
def add_books_to_cart(cart: Cart, token=Depends(verify_token)):
    ...
    try:
        user_id = token
        cart_list = add_to_cart(user_id[0], cart)
...
def delete_books_from_cart(book_id: int, token=Depends(verify_token)):
    ...
    try:
        user_id = token
        cart_list = remove_book_from_cart(user_id[0], book_id)
...
def update_books_quantity_cart(cart: Cart, quantity: int, token=Depends(verify_token)):
    ...
    try:
        user_id = token
        cart_list = update_cart(user_id[0], quantity, cart)
